...
liked:true,
        ...
        {
            ismi: "Андрей",
            picksel: "Я посещаю данный комплекс каждый месяц. Здесь чисто, нравится сервис, все хорошо. Обстановка достаточно уютная, хороший бассейн, с чистой, теплой водой. Парилка тоже отличная. Персонал внимательный и отзывчивый, девчонки симпатичные. Всегда предлагают полотенца и тапочки. Караоке, бильярдом не пользовался.",
  ...
  return (
   ...
   <h3  className="text-xl sm:text-2xl font-OpenSans font-semibold text-[#3B4255]">Отзывы посетителей</h3>
   <button className="bg-[#FF7A01] px-8 lg:px-12 py-3 rounded-xl text-base text-white font-OpenSans font-semibold hover:bg-[#ff7c01da]">Оставить отзыв</button>
   </div>
    <div className="otizivi-listes mt-6 flex flex-col gap-5">
        {
            ...
            <div className="righte">
                <p className="text-base font-OpenSans font-semibold text-[#3B4255]">{km.ismi}</p>
                <p className="text-base font-OpenSans font-normal text-[#3B4255] py-2" >{km.picksel}</p>
              <div className="otvetit flex flex-row justify-between items-center">
                <Button variant='ghost' className="!text-[#FF7A01] !text-base !font-OpenSans relative -left-3 !font-semibold" >Ответить</Button>
                <div className="liked-mkj flex flex-row gap-3 items-center">
                    <p className="text-base font-OpenSans font-normal text-[#4C4C4C]">{km.sana}</p>
        ...
        </div>
        {
            km.otzivIntoMkn.map((msk, kms)=> (
                <div key={kms} className="otziv-mkn flex flex-row gap-4 mt-5 ml-[68px]">
                <div className="lefte-ot">
                <Avatar name={msk.ismi} src={msk.rasmi} />
                </div>
                <div className="righte">
                    <p className="text-base font-OpenSans font-semibold text-[#3B4255]">{msk.ismi}</p>
                    <p className="text-base font-OpenSans font-normal text-[#3B4255] py-2" >{msk.picksel}</p>
                  <div className="otvetit flex flex-row justify-between items-center">
                    <Button variant='ghost' className="!text-[#FF7A01] !text-base !font-OpenSans relative -left-3 !font-semibold" >Ответить</Button>
                    <div className="liked-mkj flex flex-row gap-3 items-center">
                        <p className="text-base font-OpenSans font-normal text-[#4C4C4C]">{msk.sana}</p>
