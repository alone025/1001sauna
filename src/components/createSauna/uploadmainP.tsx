...
const [fileName, setFileName] = useState<string | null>(null);
  ...
  return (
    <div className="main-bandas mt-2 sm:mt-5 lg:mt-8 flex flex-col lg:flex-row gap-2 sm:gap-5">
      <div className="main-picyure w-full flex flex-col gap-5 shadow p-5 sm:p-8 rounded-2xl bg-white ">
        <h4 className="text-lg lg:text-[21px] font-OpenSans text-[#3B4255] font-semibold">
          Главные изображения
        </h4>
        <div className="image-uploader w-full">
        {fileName && imageSrc ? (
          <button className="p-4 rounded-2xl bg-[#F8F8F8] flex flex-row gap-6 justify-between w-full">
            <div className="lefte flex flex-row items-center gap-4">
                <img src={imageSrc || '/assets/buttonReplacePng.svg'} alt="img" className="h-10 w-10 rounded-lg" />
                <p className="text-sm sm:text-base line-clamp-1 font-OpenSans font-semibold text-[#3B4255]">{fileName || 'sauna.logo.png'}</p>
            </div>
            <div className="righte flex flex-row gap-1">
            <label htmlFor="uploadInpReplace" className="iconka1 cursor-pointer hover:opacity-70 transition-opacity p-3 rounded-[32px] border border-[#CCCCCC] w-max">
                    <Icon1/>
                    <input onChange={handleFileChange} type="file"  id="uploadInpReplace" className="hidden" />
                </label>
          ...
          ): ( <><label htmlFor="uploadInp" className="px-6 cursor-pointer hover:opacity-70 transition-opacity py-2 border h-[72px] w-full border-dashed rounded-2xl border-[#CCCCCC] flex flex-row gap-3 items-center justify-center">
            <svg
          ...
          <input onChange={handleFileChange} type="file"  id="uploadInp" className="hidden" /></>)}
          <p className="text-xs sm:text-sm text-[#4C4C4C] font-OpenSans font-normal mt-2">
      ...
      <div className="logotib-picture w-full flex flex-col gap-5 shadow p-5 sm:p-8 rounded-2xl bg-white ">
        <h4 className="text-lg lg:text-[21px] font-OpenSans text-[#3B4255] font-semibold">
          Логотип
        </h4>
        <div className="image-uploader w-full">
          ...
          <button className="p-4 rounded-2xl bg-[#F8F8F8] flex flex-row gap-6 justify-between w-full">
            <div className="lefte flex flex-row items-center gap-4">
                <img src={imageSrc2 || '/assets/buttonReplacePng.svg'} alt="img" className="h-10 w-10 rounded-lg" />
                <p className="text-sm sm:text-base line-clamp-1 font-OpenSans font-semibold text-[#3B4255]">{fileName2 || 'sauna.logo.png'}</p>
            </div>
            <div className="righte flex flex-row gap-1">
                <label htmlFor="uploadInpReplace2" className="iconka1 cursor-pointer hover:opacity-70 transition-opacity p-3 rounded-[32px] border border-[#CCCCCC] w-max">
                    <Icon1/>
                    <input onChange={handleFileChange2} type="file"  id="uploadInpReplace2" className="hidden" />
                </label>
                <div onClick={()=>handledelet(2)} className="ikonka2 hover:opacity-70 transition-opacity p-3 rounded-[32px] border border-[#CCCCCC] w-max">
                    <Icon2/>
                </div>
            </div>
          </button>
          ): ( <><label htmlFor="uploadInp2" className="px-6 cursor-pointer hover:opacity-70 transition-opacity py-2 border h-[72px] w-full border-dashed rounded-2xl border-[#CCCCCC] flex flex-row gap-3 items-center justify-center">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
          ...
          <input onChange={handleFileChange2} type="file"  id="uploadInp2" className="hidden" /></>)}
          <p className="text-xs sm:text-sm text-[#4C4C4C] font-OpenSans font-normal mt-2">
            Фотография (главная, не более 1024х768 пкс)*
          </p>
        </div>
